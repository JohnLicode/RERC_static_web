...
>>>>>>> kevbranch
  window.addEventListener("scroll", () => {
    ...
    if (window.scrollY > window.innerHeight * 0.5) nav.classList.add("move-to-top");
    else nav.classList.remove("move-to-top");
  });
...
<<<<<<< HEAD
  ...
  function updateButtons() { prevBtn.disabled = index === 0; nextBtn.disabled = index === slides.length - 1; }
...
<<<<<<< HEAD
  ...
  slides.forEach((slide, i) => {
    ...
    img.addEventListener("click", (e) => {
      ...
      progressBar.classList.add("hidden-during-lightbox");
    });
  });
  ...
  closeBtn.addEventListener("click", () => {
    ...
    progressBar.classList.remove("hidden-during-lightbox");
    resetImageTransform();
  });
